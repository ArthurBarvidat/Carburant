...
export default function StarBackground() {
  ...
  useEffect(() => {
    ...
    let stars: Star[] = [], nebulas: Nebula[] = [], shooters: Shooter[] = []
    ...
    function initStars() {
      ...
      for (let i = 0; i < count; i++) {
        stars.push({
          ...
          color: Math.random() < 0.3 ? 'purple' : Math.random() < 0.5 ? 'green' : 'white',
        })
      }
    }
    ...
    function initNebulas() {
      ...
      for (let i = 0; i < 6; i++) {
        nebulas.push({
          ...
          a: Math.random() * 0.04 + 0.02, da: (Math.random() - 0.5) * 0.0004,
    ...
    function maybeShoot() {
      if (Math.random() < 0.004) {
        shooters.push({
          ...
          len: Math.random() * 200 + 80, speed: Math.random() * 8 + 6,
    ...
    function draw() {
      ...
      stars.forEach(s => {
        ...
        const a = s.a.toFixed(2)
        ctx!.beginPath(); ctx!.arc(s.x, s.y, s.r, 0, Math.PI * 2)
        ctx!.fillStyle = s.color === 'purple' ? `rgba(192,132,252,${a})` : s.color === 'green' ? `rgba(52,211,153,${a})` : `rgba(241,245,249,${a})`
        ctx!.fill()
      })
