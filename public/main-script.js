...
async function loadResults(){
  ...
  if(priceTab)priceTab.classList.add("active");const fc=FC[cFuel];const distFrom=realGpsLat?"votre position GPS":cName;document.getElementById("rh-title").textContent=fc.l+" — "+cName;document.getElementById("rh-sub").textContent="Rayon 20 km · distances depuis "+distFrom;document.getElementById("rh-badge").textContent=fc.l;document.getElementById("rh-badge").style.cssText="background:"+fc.c+"22;color:"+fc.c;document.getElementById("res-loading").classList.add("show");document.getElementById("res-error").classList.remove("show");document.getElementById("res-empty").classList.remove("show");document.getElementById("res-list").innerHTML="";document.getElementById("res-count").textContent="";resetGpsBanner();const fields="id,adresse,ville,cp,geom,horaires_automate_24_24,services_service,carburants_disponibles,"+fc.f+","+fc.m;const where="within_distance(geom, geom'POINT("+cLon+" "+cLat+")', 20km)";const url="https://data.economie.gouv.fr/api/explore/v2.1/catalog/datasets/prix-des-carburants-en-france-flux-instantane-v2/records?select="+encodeURIComponent(fields)+"&where="+encodeURIComponent(where)+"&limit=50";try{const res=await fetch(url);if(!res.ok)throw new Error("Erreur ("+res.status+")");const data=await res.json();const dLat=realGpsLat||cLat,dLon=realGpsLon||cLon;resStations=(data.results||[]).map(r=>{const rl=r.geom?r.geom.lat:0,rn=r.geom?r.geom.lon:0;const svc=Array.isArray(r.services_service)?r.services_service.join(" "):(r.services_service||"");const price=r[fc.f]!=null?parseFloat(r[fc.f]):null;const dispo=Array.isArray(r.carburants_disponibles)?r.carburants_disponibles:[];const fuelInDispo=dispo.some(x=>x&&(x===cFuel||x===fc.l||x.toLowerCase().includes(cFuel.toLowerCase())));const hasPenurie=fuelInDispo&&price==null;return{id:r.id,addr:r.adresse||"",city:r.ville||"",cp:r.cp||"",lat:rl,lon:rn,dist:hav(dLat,dLon,rl,rn),is24h:r.horaires_automate_24_24==="Oui",brand:dBr(r.adresse||"",r.ville||"",svc),price,updated:r[fc.m]||null,pay:detectPay(r.services_service),hasPenurie};});if(!resStations.length){document.getElementById("res-empty").classList.add("show");return;}renderResults();}catch(e){document.getElementById("res-error-msg").textContent="❌ "+e.message;document.getElementById("res-error").classList.add("show");}finally{document.getElementById("res-loading").classList.remove("show");}}
...
async function fGeo(q){if(!q||q.length<2){fSug.classList.remove("show");return;}try{const r=await fetch("https://api-adresse.data.gouv.fr/search/?q="+encodeURIComponent(q)+"&limit=6&type=municipality");const d=await r.json();const res=(d.features||[]).map(f=>({name:f.properties.label,lat:f.geometry.coordinates[1],lon:f.geometry.coordinates[0]}));if(!res.length){fSug.classList.remove("show");return;}fSug.innerHTML=res.map((r,i)=>'<div class="ss-item" data-i="'+i+'">📍 '+esc(r.name)+'</div>').join("");fSug.classList.add("show");fSug.querySelectorAll(".ss-item").forEach((el,i)=>{el.onclick=()=>{fLat=res[i].lat;fLon=res[i].lon;fName=res[i].name;fInp.value=res[i].name;fSug.classList.remove("show");const distInfo=realGpsLat?"distances GPS précises":"distances depuis "+cName;document.getElementById("f-sub").textContent=res[i].name+" · "+distInfo;fetchFull();};});}catch{fSug.classList.remove("show");}}

async function fetchFull(){
  if(!fLat)return;
  const ff=document.getElementById("ff-fuel").value;
  if(ff==="EV"){fetchFullEV();return;}
  const rad=document.getElementById("ff-rad").value;document.getElementById("f-loading").classList.add("show");document.getElementById("f-error").classList.remove("show");document.getElementById("f-empty").classList.remove("show");document.getElementById("full-list").innerHTML="";const fields="id,adresse,ville,cp,geom,horaires_automate_24_24,services_service,gazole_prix,gazole_maj,sp95_prix,sp95_maj,sp98_prix,sp98_maj,e10_prix,e10_maj,e85_prix,e85_maj,gplc_prix,gplc_maj";const where="within_distance(geom, geom'POINT("+fLon+" "+fLat+")', "+rad+"km)";const url="https://data.economie.gouv.fr/api/explore/v2.1/catalog/datasets/prix-des-carburants-en-france-flux-instantane-v2/records?select="+encodeURIComponent(fields)+"&where="+encodeURIComponent(where)+"&limit=80";try{const res=await fetch(url);if(!res.ok)throw new Error("Erreur ("+res.status+")");const data=await res.json();fStations=(data.results||[]).map(r=>{const rl=r.geom?r.geom.lat:0,rn=r.geom?r.geom.lon:0;const prices=[];for(const[k,c]of Object.entries(FC)){const v=r[c.f];if(v!=null&&v!==""&&!isNaN(parseFloat(v)))prices.push({fuel:k,price:parseFloat(v),updated:r[c.m]||null});}const svc=Array.isArray(r.services_service)?r.services_service.join(" "):(r.services_service||"");return{id:r.id||(rl+"_"+rn),addr:r.adresse||"",city:r.ville||"",cp:r.cp||"",lat:rl,lon:rn,dist:hav(realGpsLat||cLat,realGpsLon||cLon,rl,rn),is24h:r.horaires_automate_24_24==="Oui",brand:dBr(r.adresse||"",r.ville||"",svc),prices,pay:detectPay(r.services_service)};});renderFull();}catch(e){document.getElementById("f-error-msg").textContent="❌ "+e.message;document.getElementById("f-error").classList.add("show");}finally{document.getElementById("f-loading").classList.remove("show");}}
